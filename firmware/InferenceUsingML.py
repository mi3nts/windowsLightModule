...
#import joblib is enough for loading model
import joblib
...
BUFFER_SECONDS = 60
MODEL_PATH = 'Sat_split_cp_model.pkl'
...
def read_new_data(sensor, path):
    ...
    try:
        if not os.path.exists(path):
            # File might not exist yet early in the day
            return pd.DataFrame()
        with open(path, 'r') as f:
...
while True:
    ...
    # Find latest valid timestamp for AS7265X sensor (you can choose other logic if needed)
